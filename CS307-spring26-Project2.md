...
- **The Advance part:** Totals 20 points.
  - The Advance sections are in **2.2 Join Operators and Advanced SeqScan Calculations** and **3. Index**. Points can be transferred between these sections, but the maximum overflow is 5 points.
  - **4. Transaction** is worth 8 points and can only be overflowed into the Advance sections.
- For bugs in the original code, you can submit a **pull request**, and you can get score for advanced part.
- **Q&A during the presentation part:** This part includes but is not limited to the following forms:
  - Answering questions, such as multiple-choice questions.
  - Explaining the design process.
  - Rewriting code on the spot.

If the answer is incorrect, no points will be awarded for this part and its related basic and advanced requirements.
...
2. Clock Replacer
   - Implement the PageReplacer Interface and complete `Victim()`, `Pin(int frameId)`, and `Unpin(int frameId)` methods in `ClockReplacer`, and pass the `ClockReplacerTest` JUnit tests.
...
- Support `DELETE` operations, with full support for conditions (AND, OR, equality, and range filters). (10 points). **Q&A during the presentation**
...
- Support `Count` operations, with full support for conditions (AND, OR, equality, and range filters). (10 points)
...
- **Q&A during the presentation** (Failure to this requirement, you will result in no points in Task 2- Advance.)
...
1. Index Support:
   - Recognize the `create index xx on t(col)` statement, `drop index xx` statement, and the JSON file will be modified accordingly.
   - Within the project framework, build an in-memory B+ tree based on the index, and be able to print each node.
   - Support creating multiple indexes and building multiple B+ trees.
   - Support dynamically creating B+ trees during a single run of the project, with `insert` and `delete` operations.
2. Validation:
...
Implement the `rollback()`, `savepoint(String savepointName)`, `rollbackToSavepoint(String savepointName)`, and `releaseSavepoint(String savepointName)` methods in the `TransactionManagerApi`, and complete all test cases in `TransactionManagerTest`.

**Q&A during the presentation.** (Failure to this requirement, you will result in no points in Task 4.)

- Explain how to design snapshot in `begin` command.
- Explain if executing `commit` command, what happens in **physical and logical** structure.
